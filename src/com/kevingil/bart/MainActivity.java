...
public class MainActivity extends Activity {
	...
	ArrayList<TextView> timerViews = new ArrayList<TextView>();
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        if(prefs.getBoolean("first_timer", true)){
			...
			greetingTv.setTextColor(getResources().getColor(R.color.black));
        ...
        stationSuggestions = new ArrayList<StationSuggestion>();
    ...
    @Override public boolean onCreateOptionsMenu(Menu menu) {
    	//Setting up action bar only, level less than 13 no longer supported 
...
@Override public boolean onOptionsItemSelected(MenuItem item) {
    ...
    public void displayRouteResponse(routeResponse routeResponse){
    	...
    	timerViews = new ArrayList<TextView>(); // release old ETA text views
    	maxTimer = 0;
    	try{
	    		...
	    		tr.setOnClickListener(new OnClickListener(){
					...
					public void onClick(View arg0) {
						...
						if (!thisRoute.isExpanded){ // if route not expanded
							...
							arrivalTv.setTextColor(getResources().getColor(R.color.black));
		...
		for(int x=0;x<etdResponse.etds.size();x++){
			...
			if (thisEtd.destination != lastDestination){ // new train destination
				...
				if (numAlt == 1)	//0xFFF06D2F  C9C7C8
					nextTimeTv.setTextColor(getResources().getColor(R.color.black));
				else if (numAlt == 2)
					nextTimeTv.setTextColor(getResources().getColor(R.color.black));
				long counterTime = thisEtd.minutesToArrival * 60*1000;
    ...
    private BroadcastReceiver serviceStateMessageReceiver = new BroadcastReceiver() {
    	  ...
    	  public void onReceive(Context context, Intent intent) {
    	    ...
    	    if(status == 0){ // service stopped
    	    	Log.d("MainActivity-BroadcastReceived", "service stopped");
    	    	stopServiceTv.setVisibility(View.GONE);
    	    }
    	    else if(status == 1){ // service started
    	    	Log.d("MainActivity-BroadcastReceived", "service started");
    	    	...
    	    	Log.d("MainActivity-BroadcastReceived", "countdown timer expired");
    	    	...
    	    	Log.d("MainActivity-BroadcastReceived", "requestTask complete");
    	    	...
    	    	Log.d("MainActivity-BroadcastReceived", "Bart parser complete");
	...
	private boolean usherServiceIsRunning() {
	    ...
	    for (RunningServiceInfo service : manager.getRunningServices(Integer.MAX_VALUE)) {
	        if ("com.kevingil.bart.UsherService".equals(service.service.getClassName())) {
	            return true;
	        }
	    }
	    return false;
	...
	private routeResponse removeExpiredRoutes(routeResponse response){
		...
		Date now = new Date();
		// Might not actually be an Integer
		ArrayList<Integer> indexesToRemove = new ArrayList<Integer>(response.routes.size());
